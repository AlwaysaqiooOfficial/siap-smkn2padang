...
export async function updateStudent(id: string, input: UpdateStudentInput, scopedClassId?: string) {
  ...
  const { parentEmail, parentFullName, parentPhone, parentAddress, ...studentData } = input;
  ...
  const parentChanged = parentEmail !== undefined || parentFullName !== undefined || parentPhone !== undefined || parentAddress !== undefined;
  if (parentChanged) {
    const currentParent = (student as any).parent;
    updateData.parentEmail = parentEmail ?? student.parentEmail ?? currentParent?.email ?? currentParent?.user?.email ?? "";
    updateData.parentFullName = parentFullName ?? student.parentFullName ?? currentParent?.fullName ?? "";
    updateData.parentPhone = parentPhone ?? student.parentPhone ?? currentParent?.phone ?? "";
    updateData.parentAddress = parentAddress ?? student.parentAddress ?? currentParent?.address ?? "";
  }
